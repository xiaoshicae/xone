...
1. **`go.mod` 的 module 路径必须带 `/v3` 后缀**
   ```
   module github.com/xiaoshicae/xone/v3
   ```

2. **所有内部 import 必须带 `/v3`**
   ```go
...
3. **下游引用也必须带 `/v3`**
   ```
   require github.com/xiaoshicae/xone/v3 v3.x.x
   ```

### 为什么？

Go module 通过路径区分大版本。不加 `/vN`，Go 工具链会认为只有 v0/v1 版本可用，导致：
...
1. 修改 `go.mod`：`module github.com/xiaoshicae/xone/v4`
2. 全局替换 import：`/v3/` → `/v4/`
3. 验证编译：`go build ./...`
4. 打 tag：`git tag v4.0.0 && git push origin v4.0.0`

## 禁止事项

- **禁止** v2+ 的 module 路径不带 `/vN` 后缀
- **禁止** tag 与 go.mod 中的主版本号不一致（如 go.mod 写 `/v3` 但打 `v4.0.0` tag）
