package xgorm

import (
	"context"
	"fmt"
	"maps"
	"slices"
	"strings"

	"gorm.io/gorm"
)

// C 获取 gorm client，支持指定 client name 获取，name 为空则取默认 client
//
// 推荐使用 CWithCtx()，保证 ctx 中内容能传递到下游(trace 等)。
//
// 找不到时 panic。返回 nil 并不会让程序走得更远——*gorm.DB 的任何方法在 nil 上
// 都是空指针解引用，只是把同一个 panic 推迟到调用方第一次用它的时候，
// 而那里的栈里只剩 "invalid memory address"，看不出根因是配置没配。
// 这是启动期的配置问题，不是运行期需要处理的错误。
//
// 可选依赖（配了就用、没配就跳过）用 Has() 先判断。
func C(name ...string) *gorm.DB {
	client := get(name...)
	if client == nil {
		panic(noClientMsg(name...))
	}
	return client
}

// CWithCtx 可以保证 ctx 中内容能传递到下游(trace 等)
//
// 找不到 client 时的行为同 C。
func CWithCtx(ctx context.Context, name ...string) *gorm.DB {
	return C(name...).WithContext(ctx)
}

// Has 报告指定 client 是否已配置
//
// 供可选依赖使用：配了就用、没配就跳过，不必用 C 去触发 panic。
func Has(name ...string) bool {
	return get(name...) != nil
}

// noClientMsg 拼装 panic 信息，带上已配置的 client 名称
//
// 只报"没找到"帮助有限：名字写错和整个 XGorm 没配是两个不同的问题，
// 列出实际配了哪些，两者一眼可分。
func noClientMsg(name ...string) string {
	want := "default"
	if len(name) > 0 {
		want = name[0]
	}

	configured := clientNames()
	if len(configured) == 0 {
		return fmt.Sprintf("XOne xgorm: no client found for name=[%s], no client configured at all, check the XGorm section of your config", want)
	}
	return fmt.Sprintf("XOne xgorm: no client found for name=[%s], configured=[%s]", want, strings.Join(configured, " "))
}

// clientNames 返回已配置的 client 名称，不含内部默认别名
func clientNames() []string {
	clientMu.RLock()
	defer clientMu.RUnlock()

	names := slices.Sorted(maps.Keys(clientMap))
	return slices.DeleteFunc(names, func(n string) bool { return n == defaultClientName })
}
