...
| 区间 | 成员 | Order | 含义 |
|------|------|-------|------|
| **框架保留区**（负值） | xconfig | -100 | 最先启动（配置必须先于一切就绪）；它没有关闭钩子 |
| | xlog | -50 | 次先启动、最后关闭，使任何模块在启停两端打的日志都能落盘 |
| **业务区**（>= 0） | 其余模块 / 用户资源 | 100（默认） | 相互之间按 init 顺序，关闭时逆序 |

保留区**不是约定，是强制的**：

- `xhook.Order(n)` 对 `n < 0` 直接 panic，业务 Hook 进不了负值区
- 负值只能通过 `xhook.ReservedOrder` 设置，其参数类型在 `internal/hookorder` 中，
  外部模块 import 会直接编译失败（`use of internal package ... not allowed`）

因此业务 Hook 不可能先于 xconfig 启动，也不可能晚于 xlog 关闭 —— 保持默认 Order 即可，
关闭逻辑里可以放心打日志。业务区内部若需相对顺序，用 0 ~ 正整数自行编排。
...
```
注册：数据库(Order=100)  缓存(Order=100)  日志(保留区 -50)
启动：日志 → 数据库 → 缓存
...
框架内部只有 `xconfig`（-100）和 `xlog`（-50）声明了 Order，且两者都不是可选的：
Go 的 init 顺序按 import path 字典序排，用户模块叫 `acme/...` 还是 `myapp/...`
就决定了它排在 xone 之前还是之后——这不是使用者能通过 import 纪律控制的。
没有配置层级，路径靠前的用户包会在配置加载完成前就执行 BeforeStart；
没有日志层级，这样的用户包会在日志写入器关闭之后才关闭，其关闭日志直接丢失。
把这两层放进强制保留区，才使这个保证不依赖使用者的自觉。
