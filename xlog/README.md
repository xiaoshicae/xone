...
**轮转失败不丢日志。** 切换日志文件失败时（磁盘满、权限变更），写入器保留原文件句柄
继续写，并按分钟级间隔打一条 warn，下一条日志再尝试轮转。直接返回错误的话，
从第一次失败起每条日志都会走同一条失败路径，而异步写入器只保留第一个错误、
且只在关闭时返回 —— 现象就是某天日志突然断了，没有任何报错。

**关闭有超时。** 底层写入卡住时（磁盘满、NFS 不响应），关闭最多等待 5 秒，
之后放弃缓冲区里剩余的日志并返回 `async writer drain timeout`。xlog 是最后关闭的
模块，无限期等在这里等于整个进程退不出去。
